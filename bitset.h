...
#define bitset_size_to_bits(size) size / (sizeof(bitset_index_t) * CHAR_BIT) + 1 + (size % (sizeof(bitset_index_t) * CHAR_BIT) != 0)
...
#define bitset_setbit(array, index, expression) do {  \
    bitset_index_t location = index + sizeof(bitset_index_t) * CHAR_BIT;\
    \
    int array_index = location / (sizeof(bitset_index_t) * CHAR_BIT);\
    \
    if (location >= bitset_size(array) + sizeof(bitset_index_t) * CHAR_BIT) {\
        /* FIXME use error.h */ \
        error_exit("%s: Byly překročeny hranice pole!", MODUL_NAME); \
    }\
    else {\
        bitset_index_t val = 1L << (location % (sizeof(bitset_index_t) * CHAR_BIT));\
        if (expression) {\
...
#define bitset_getbit(array, index) ( \
    (index + sizeof(bitset_index_t) * CHAR_BIT >= bitset_size(array) + sizeof(bitset_index_t) * CHAR_BIT) ? \
    error_exit("%s: Byly překročeny hranice pole!", MODUL_NAME),1 :\
    /* lisp moment */ \
    (int) (1 & (array[/*array_index */ ((/*location: */ index + sizeof(bitset_index_t) * CHAR_BIT) / (sizeof(bitset_index_t) * CHAR_BIT))] >> ((/* location: */ index + sizeof(bitset_index_t) * CHAR_BIT) % ( /* size in bits */ sizeof(bitset_index_t) * CHAR_BIT)))) \
    )
...
inline int bitset_getbit(bitset_t array, bitset_index_t index) {
    ...
    bitset_index_t value = array[array_index] >> (location % (sizeof(bitset_index_t) * CHAR_BIT));
    return 1 & value;
}
